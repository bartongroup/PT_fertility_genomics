...
def _extract_gene_from_fasta_headers(headers: str) -> Optional[str]:
    """
    Extract a gene symbol from MaxQuant 'Fasta headers'.

    Supports multiple header styles, including:
    - UniProt style: GN=GENE
    - Custom style: Gene_Symbol=Gene

    Parameters
    ----------
    headers : str
        'Fasta headers' field (often ';' separated).
    ...
    # Try custom token first (as seen in your file)
    m = re.search(r"\bGene_Symbol=([A-Za-z0-9\-]+)\b", s)
    if m:
        return _normalise_gene_symbol(m.group(1))

    # Fallback: UniProt token
    m = re.search(r"\bGN=([A-Za-z0-9\-]+)\b", s)
    if m:
        return _normalise_gene_symbol(m.group(1))
...
def build_gene_level_table(
    ...
    df["gene_key"] = df["Fasta headers"].map(_extract_gene_from_fasta_headers)
    ...
    peptide_prefix = "Peptides "
    sample_cols = _find_sample_columns(df=df, prefix=peptide_prefix)
    if not sample_cols:
